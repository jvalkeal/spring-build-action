...
function getDownloadInfo(
  ...
  refs.forEach(ref => {
    if (semver.satisfies(ref, version)) {
      ...
      versionMap.set(
        ref,
        `${mavenMirror}gradle-${ref}-bin${extension}`
      );
    }
  });
  ...
  if (curUrl == '') {
    throw new Error(
      `No valid download found for version ${version}. Check ${mavenMirror} for a list of valid versions or download your own maven file and add the maven-file argument`
    );
  }
